...
function App() {
  ...
  return <main>
    ...
    <header className="site-header">
      ...
      <nav className={menuOpen ? 'nav-links nav-open' : 'nav-links'} aria-label="主导航">{site.navigation.map((item) => <a className={activeSection === item.href.slice(1) ? 'is-active' : ''} aria-current={activeSection === item.href.slice(1) ? 'page' : undefined} key={item.href} href={item.href} onClick={closeMenu}>{item.label}</a>)}<a className="mobile-resume" href="/liuxinyi_resume.pdf?v=20260912" download="北邮-刘欣怡-简历.pdf" onClick={closeMenu}><Download size={16} /> 下载简历</a></nav>
      <a className="header-resume" href="/liuxinyi_resume.pdf?v=20260912" download="北邮-刘欣怡-简历.pdf"><Download size={16} /> 下载简历</a>
    </header>
    {menuOpen && <button className="nav-scrim" type="button" aria-label="关闭导航" onClick={closeMenu} />}
