...
function App() {
  ...
  return <main>
    ...
    <section className="hero" id="top">
      ...
      <motion.div className="avatar-stage" onPointerEnter={playAvatarOnHover} onPointerLeave={() => avatarVideo.current?.pause()} initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.9, delay: 0.15, ease: [0.22, 1, 0.36, 1] }}><div className="avatar-halo" aria-hidden="true" /><motion.div className="avatar-wrap" animate={reduceMotion ? {} : { y: [0, -13, 0], rotate: [0, -1, 0, 1, 0] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}><video ref={avatarVideo} className="avatar-video" poster="/sylvia-avatar.webp" muted playsInline preload="auto" onEnded={() => avatarVideo.current?.pause()} aria-label="Sylvia 的动态粉黑风格 3D 玩偶形象"><source src="/sylvia-avatar.mp4" type="video/mp4" /><img src="/sylvia-avatar.webp" alt="Sylvia 的粉黑风格 3D 玩偶形象" /></video></motion.div><motion.div className="sticker sticker-now" animate={reduceMotion ? {} : { y: [0, -7, 0], rotate: [2, -2, 2] }} transition={{ duration: 4.5, repeat: Infinity }}><span>NOW</span><strong>building<br />with AI</strong></motion.div><motion.div className="sticker sticker-spark" animate={reduceMotion ? {} : { rotate: [0, 12, 0] }} transition={{ duration: 4, repeat: Infinity }}><Sparkles size={19} /></motion.div></motion.div>
    </section>
